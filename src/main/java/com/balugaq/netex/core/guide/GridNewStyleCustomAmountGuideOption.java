...
public class GridNewStyleCustomAmountGuideOption implements SlimefunGuideOption<Integer> {
    ...
    public @NotNull Optional<ItemStack> getDisplayItem(@NotNull Player p, ItemStack guide) {
        ...
        ItemStack item = new CustomItemStack(
            Material.FURNACE,
            "&aQuantidade personalizada de retirada da Grade Avançada",
            "",
            "&7Quantidade atual: " + value + " (intervalo permitido: 1~" + GRID_NEW_STYLE_MAX_CUSTOM_AMOUNT + ")",
            "&7\u21E8 &eClique pra definir a quantidade"
        );
    ...
    public void onClick(@NotNull Player p, @NotNull ItemStack guide) {
        p.closeInventory();
        p.sendMessage(ChatColors.color("&eDigite a quantidade personalizada de retirada da Grade Avançada"));
        ChatInput.waitForPlayer(Networks.getInstance(), p, s -> {
            try {
                ...
                if (value < 1 || value > GRID_NEW_STYLE_MAX_CUSTOM_AMOUNT) {
                    p.sendMessage("Digite um número inteiro positivo entre 1 e " + GRID_NEW_STYLE_MAX_CUSTOM_AMOUNT);
                    return;
                }
                ...
                p.sendMessage("Digite um número inteiro positivo entre 1 e " + GRID_NEW_STYLE_MAX_CUSTOM_AMOUNT + e.getMessage());
            }
        });
    }
